...
class IdeaListCommand extends Command {
    ...
    runInternal(msg, args) {
        ...
        if (!userIdeas || userIdeas.length === 0) {
            ...
            } else if ((user.id === '269615032374001668')) {
                msg.channel.send("Wolfy keeps his ideas on Trello now as well. Well, I guess everyone is doing it now?\nhttps://trello.com/b/ociwautd/the-8-bit-wolfs-mods");
            } else {
                if (user.id === msg.author.id) {
            ...
            if (user.id === msg.author.id || !IdeaService.isOptout(user.id)) {
                ...
                let i = 1;
                msg.channel.send(userIdeas.map(idea => (i++) + ".: " + idea).join("\n"));
            } else {
